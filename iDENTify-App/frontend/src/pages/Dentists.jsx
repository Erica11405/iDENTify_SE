...
//     const updated = { ...dentist, operatingHours: { ...dentist.operatingHours, [field]: value } };
//     saveDentistChanges(updated);
//   };

//   const updateLunch = (dentist, field, value) => {
//     const updated = { ...dentist, lunch: { ...dentist.lunch, [field]: value } };
//     saveDentistChanges(updated);
//   };
...
//         {/* BLUE PLUS BUTTON BESIDE AVAILABILITY */}
//         <div className="filter-group">
...
function Dentists() {
  ...
  const saveDentistChanges = async (updatedDentist) => {
    // 1. Instantly update the UI/Store
    updateDentistInStore(updatedDentist);
    
    // 2. Clear any existing timer for this specific dentist
    if (debounceRef.current[updatedDentist.id]) {
      clearTimeout(debounceRef.current[updatedDentist.id]);
    }
    
    // 3. Set a new timer to save to the database (increased to 1000ms for time inputs)
  ...
  const updateOperatingHours = (dentist, field, value) => {
    ...
    const updated = { 
      ...dentist, 
      operatingHours: { ...currentHours, [field]: value } 
    };
    saveDentistChanges(updated);
  };
  ...
  const updateLunch = (dentist, field, value) => {
    ...
    const updated = { 
      ...dentist, 
      lunch: { ...currentLunch, [field]: value } 
    };
    saveDentistChanges(updated);
  };
