...
//           const familyIds = familyData.map(m => m.id);
...
//       // 3. Fetch Appointments for ALL IDs (Parallel Requests)
...
//       // 4. Sort: Newest/Upcoming first
//       allAppts.sort((a, b) => new Date(a.appointment_datetime) - new Date(b.appointment_datetime));
...
//           <Ionicons name="calendar-clear-outline" size={64} color="#CBD5E1" />
//           <Text style={styles.emptyTitle}>No appointments found</Text>
//           <Text style={styles.emptySubtitle}>Book for yourself or family members.</Text>
//         </ScrollView>
//       ) : (
//         <FlatList
//           data={appointments}
...
//                 <View style={styles.info}>
//                   {/* --- NEW: PATIENT NAME BADGE --- */}
//                   <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
//                     <Ionicons name="person" size={12} color="#64748B" style={{ marginRight: 4 }} />
//                     <Text style={{ fontSize: 12, color: "#64748B", fontWeight: "600" }}>
//                       {item.full_name} {/* Shows Patient Name */}
//                     </Text>
//                   </View>
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (familyRes.ok) {
          const familyData = await familyRes.json();
          // SAFEGUARD: Ensure familyData is an array before mapping
      ...
      const promises = allPatientIds.map(id =>
        fetch(`${API.appointments}?patient_id=${id}`)
          .then(r => r.json())
          // SAFEGUARD: If backend returns an error object, convert to empty array
          .then(data => Array.isArray(data) ? data : [])
          .catch(() => []) // Catch network failures per request
      );

      const results = await Promise.all(promises);
      let allAppts = results.flat();

      // 4. Sort: Newest/Upcoming first (Safeguard date sorting)
      allAppts.sort((a, b) => {
        const dateA = a.appointment_datetime ? new Date(a.appointment_datetime) : new Date(0);
        const dateB = b.appointment_datetime ? new Date(b.appointment_datetime) : new Date(0);
        return dateB - dateA;
      });
  ...
  return (
    <View style={styles.container}>
      ...
      {loading && !refreshing ? (
        ...
        <FlatList
          data={appointments}
          // SAFEGUARD: Use fallback key if item.id is missing
          keyExtractor={(item, index) => item.id ? item.id.toString() : `fallback-${index}`}
          contentContainerStyle={styles.listContent}
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={["#1B93D5"]} />}
          renderItem={({ item }) => {
            const statusStyle = getStatusColor(item.status);
            const dateObj = item.appointment_datetime ? new Date(item.appointment_datetime) : new Date();

            return (
              <TouchableOpacity
                style={styles.card}
                activeOpacity={0.7}
                onPress={() => item.id && router.push(`/appointments/${item.id}`)}
              >
                <View style={styles.dateBox}>
                  <Text style={styles.dayText}>{dateObj.getDate()}</Text>
                  <Text style={styles.monthText}>{dateObj.toLocaleDateString(undefined, { month: 'short' })}</Text>
