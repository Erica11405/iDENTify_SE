...
import {
  ...
  Alert, ScrollView, Platform, Modal, Pressable
} from "react-native";
import { useUser } from "@clerk/clerk-expo";
import { useState, useEffect } from "react";
import { useRouter } from "expo-router";
...
export default function EditProfile() {
      ...
      try {
        ...
        if (data) {
          ...
          if (nameParts.length > 0) {
            if (nameParts.length === 1) {
              setGivenName(nameParts[0]);
              setMiddleName(""); // [FIXED] Reset state 
              setLastName(""); // [FIXED] Reset state
            } else if (nameParts.length === 2) {
              setGivenName(nameParts[0]);
              setMiddleName(""); // [FIXED] Reset state
              setLastName(nameParts[1]);
          ...
          if (data.birthdate) {
            ...
            setBday(d.toISOString().split('T')[0]);
            setDateObject(d);
            setDisplayAge(`${calculateAge(d)} years old`);
          }
        } else {
          setGivenName(user.firstName || "");
          setLastName(user.lastName || "");
  ...
  const saveChanges = async () => {
    ...
    const payload = {
      full_name: fullNameCombined,
      first_name: givenName, // [FIXED] Added explicit sub-fields 
      middle_name: middleName,
      last_name: lastName,
      gender: sex,
      birthdate: bday,
      ...
      xrays: xrays,
      parent_id: parentId,
      vitals: { ...(patient?.vitals || {}), age: calculateAge(dateObject) }
    };
