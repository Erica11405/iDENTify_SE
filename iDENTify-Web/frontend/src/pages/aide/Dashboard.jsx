...
function AideDashboard() {
  const api = useApi();
  const { appointments, queue, patients } = useAppStore();
  ...
  return (
    <div className="dashboard-page">
      <div className="dashboard-header">
        <h1 className="dashboard-title">Clinic Management</h1>
        <p className="dashboard-subtitle-main">Overview of today's clinic activity.</p>
      </div>
      ...
      <div className="dashboard-main-content">
        <div className="dashboard-section">
          <h3 className="section-title">Current Queue Status</h3>
          <div className="table-container">
            <table>
              ...
              <tbody>
                {queue.map(q => (
                  <tr key={q.id}>
                    <td className="bold-text">{q.patient_name || q.full_name}</td>
                    <td>{q.assigned_dentist || 'Unassigned'}</td>
                    <td>
                      <span className={`badge ${q.status?.toLowerCase().replace(' ', '-') || 'waiting'}`}>
                        {q.status}
                      </span>
                    </td>
                  </tr>
