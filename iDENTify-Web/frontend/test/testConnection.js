...
async function testConnection() {
  ...
  try {
    ...
    if (healthRes.ok) {
      ...
      console.log(`❌ Failed: Backend returned ${healthRes.status} ${healthRes.statusText}`);
    }
    ...
    if (apiRes.ok) {
      ...
      console.log(`❌ Failed: API returned ${apiRes.status} ${apiRes.statusText}`);
    ...
    console.log('\nPossible causes:');
    console.log('1. Backend server is not running (npm run dev in backend folder)');
    console.log('2. Backend is running on a different port (check server.js or .env)');
