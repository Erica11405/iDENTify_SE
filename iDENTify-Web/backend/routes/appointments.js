...
function parseTime(dateTimeStr) {
  ...
  return dateTimeStr; // Fallback if already formatted
}

router.post("/", async (req, res) => {
  // We check for 'procedure' OR 'services' to be safe
  const { patient_id, dentist_id, timeStart, procedure, services, notes, status } = req.body;
  const appointment_datetime = parseTime(timeStart);
  ...
  // Combine services array into a single string for the 'reason' column
...
// Keep your other GET, PUT, DELETE routes below this...
module.exports = router;