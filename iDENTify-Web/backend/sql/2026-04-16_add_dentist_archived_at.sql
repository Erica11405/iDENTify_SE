...
SET @add_is_archived_sql = IF(
  @dentists_table_exists = 0 OR @has_is_archived > 0,
...
SET @add_archived_at_sql = IF(
  @dentists_table_exists = 0 OR @has_archived_at > 0,
...
SET @backfill_archived_at_sql = IF(
  @dentists_table_exists > 0 AND @has_is_archived > 0 AND @has_archived_at > 0,
...
SET @verify_dentists_snapshot_sql = IF(
  @dentists_table_exists = 0,
  'SELECT ''skip dentists snapshot (table missing)'' AS message',
  'SELECT
...
PREPARE stmt_verify_dentists_snapshot FROM @verify_dentists_snapshot_sql;
EXECUTE stmt_verify_dentists_snapshot;
DEALLOCATE PREPARE stmt_verify_dentists_snapshot;